...
export function castingSlots(document, name, keys, options, t = sheetTranslator()) {
    const select = el(document, "select", "");
    select.setAttribute("aria-label", t("spells.castingSlot", { name }));
...
export function grantedSpells(view, catalog, options, change) {
    ...
    root.append(el(document, "h3", "", t("spells.grants")));
    for (const choice of rows(options["castingAbilityChoices"])) {
        ...
        select.setAttribute("aria-label", t("spells.castingAbility", { name }));
        select.disabled = !editable;
        select.append(option(document, "", t("builder.chooseAbility")));
    ...
    for (const choice of rows(options["pendingChoices"])) {
        ...
        group.append(el(document, "legend", "", t("spells.grantCount", { name, picked: picked.length, count: String(choice["choose"]) })));
        for (const ref of picked)
            group.append(button(document, t("play.remove", { name: catalog.find(item => item.id === ref)?.name ?? ref }), () => change({ operation: "select-grant-spell", key, ref, selected: false }), !editable));
        const controls = el(document, "div", "dnd-workflow-controls"), select = el(document, "select", "");
        select.setAttribute("aria-label", t("spells.choice", { key }));
        select.disabled = !editable;
        ...
        controls.append(select, button(document, t("spells.choose"), () => change({ operation: "select-grant-spell", key, ref: select.value, selected: true }), !editable || !select.options.length || picked.length >= Number(choice["choose"])));
    ...
    for (const grant of rows(options["granted"])) {
        ...
        select.disabled = !editable;
        controls.append(select, button(document, t("spells.castGranted"), () => change({ operation: "cast-granted-spell", key, slot: select.value }), !editable || select.selectedIndex < 0));
        row.append(controls);
        root.append(row);
    }
    if (root.children.length === 1)
        root.append(el(document, "p", "dse-empty", t("spells.noGrants")));
    return root;
}
export function copySpellForm(document, state, spell, classId, cost, submit, t = sheetTranslator()) {
    ...
    scroll.setAttribute("aria-label", t("spells.consumeScroll"));
    scroll.append(option(document, "", t("spells.copyBook")));
    ...
    for (const item of state.inventory)
        if (item.qty > 0 && (item["spellRef"] === spell.id || item.name.toLocaleLowerCase().includes("scroll") && item.name.toLocaleLowerCase().includes(foldedName)))
            scroll.append(option(document, item.id, t("spells.scrollCount", { name: item.name, count: item.qty })));
    const apply = button(document, t("spells.reviewCopy"), () => { }, Number(state.currency["gp"] ?? 0) < cost);
    apply.type = "submit";
...
export function swapSpellForm(document, state, classId, eligible, catalog, submit, t = sheetTranslator()) {
    ...
    from.setAttribute("aria-label", t("spells.replace"));
    to.setAttribute("aria-label", t("spells.replacement"));
    ...
    const apply = button(document, t("spells.reviewSwap"), () => { }, !from.options.length || !to.options.length);
    apply.type = "submit";
    form.addEventListener("submit", event => { event.preventDefault(); submit({ operation: "swap-spell", classId, out: from.value, ref: to.value }); });
    form.append(el(document, "p", "", t("spells.swapHelp")), from, to, apply);
    return form;
}
export function spellSwapHistory(view, catalog) {
    ...
    section.append(el(document, "summary", "", t("spells.history", { count: state.spellSwaps.length })));
    state.spellSwaps.forEach((swap, index) => {
        ...
        if (editable)
            row.append(button(document, t("spells.removeHistory"), () => save(draft => { draft.spellSwaps.splice(index, 1); })));
        section.append(row);
    });
    return section;
}
