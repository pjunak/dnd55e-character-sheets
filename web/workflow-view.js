...
const categoryNames = { weapon: "Weapons", armor: "Armor", gear: "Adventuring gear", tool: "Tools", "magic-item": "Magic items", pack: "Equipment packs" };
const facetFields = { weapon: "category", armor: "armorType", tool: "type", "magic-item": "rarity" };
...
export function equipmentPicker(document, catalog, unavailable, dirty, commit) {
    const root = el(document, "div", "dnd-equipment-picker");
    const controls = el(document, "div", "dnd-workflow-controls");
    const category = el(document, "select", "");
    category.setAttribute("aria-label", "Equipment category");
    category.append(option(document, "", "All equipment"));
    for (const kind of equipmentKinds)
        category.append(option(document, kind, categoryNames[kind]));
    const facet = el(document, "select", "");
    facet.setAttribute("aria-label", "Equipment group");
    ...
    const renderTray = () => {
        tray.replaceChildren(el(document, "h4", "", `Selected items · ${cart.length}`));
        ...
        for (const item of cart) {
            ...
            qty.setAttribute("aria-label", `Selected ${item.name} quantity`);
            qty.addEventListener("change", () => { item.qty = Math.min(5000, Math.max(1, Math.trunc(qty.valueAsNumber || 1))); qty.value = String(item.qty); });
            row.append(qty, button(document, "×", () => { cart.splice(cart.indexOf(item), 1); renderTray(); }, false, `Remove selected ${item.name}`));
            tray.append(row);
        }
        if (!cart.length)
            tray.append(el(document, "p", "dse-empty", "Choose items, then add the whole selection together."));
    };
    const stage = (source, name) => {
        ...
        if (existing)
            ...
            cart.push({ id: createId("item"), name, qty: 1, location: "pack", notes: "", ...(source ? { kind: source.kind, ref: source.id, snapshot: structuredClone(source) } : {}) });
        renderTray();
    };
    const renderResults = () => {
        ...
        for (const item of filtered.slice(0, 80)) {
            ...
            row.append(details, button(document, "＋", () => stage(item, item.name ?? item.id), false, `Select ${item.name ?? item.id}`));
            results.append(row);
        }
    };
    const renderFacets = () => {
        ...
        if (field)
            for (const value of [...new Set(catalog.filter(item => item.kind === category.value).map(item => item[field]).filter((value) => typeof value === "string" && value.length > 0))].sort())
                facet.append(option(document, value, label(value)));
        facet.disabled = facet.options.length === 1;
        renderResults();
    };
    category.addEventListener("change", renderFacets);
    facet.addEventListener("change", renderResults);
    search.addEventListener("input", renderResults);
    const custom = el(document, "form", "dnd-workflow-controls");
    const name = el(document, "input", "");
    name.required = true;
    name.maxLength = 200;
    name.placeholder = "Custom item name";
    name.setAttribute("aria-label", "Custom item name");
    const customAdd = button(document, "Select custom item", () => { });
    ...
    if (unavailable)
        root.append(el(document, "p", "dse-empty", "The equipment catalog is unavailable. You can still add custom items."));
    split.append(results, tray);
    root.append(controls, split, custom, add);
    renderFacets();
    renderTray();
    return root;
}
export function restControls(view, available, change) {
    ...
    for (const rest of ["short", "long"])
        buttons.append(button(document, rest === "short" ? "Short rest" : "Long rest", () => change({ operation: "rest", rest }, true), !editable || !available));
    ...
    if (!available)
        controls.append(el(document, "p", "dse-empty", "Connect a rules engine to calculate rests. HP and resource counters remain editable."));
    return controls;
}
export function playReview(document, before, after) {
    ...
    if (ended.length)
        review.append(el(document, "p", "", `Active features ended: ${ended.length}`));
    return review;
}
export function spellBrowser(view, catalog, computed, browser, change) {
    const { document, state, editable } = view;
    const root = el(document, "section", "dnd-spell-browser dse-section");
    root.append(el(document, "h3", "", "Manage class spells"));
    const casters = rows(record(computed["spellcasting"])["perClass"]);
    if (!casters.length) {
        root.append(el(document, "p", "dse-empty", "Choose a spellcasting class in Builder first."));
        return root;
    }
    ...
    classSelect.setAttribute("aria-label", "Spellcasting class");
    ...
    level.setAttribute("aria-label", "Spell level");
    level.append(option(document, "", "All levels"));
    for (let i = 0; i <= 9; i++)
        level.append(option(document, String(i), i === 0 ? "Cantrips" : `Level ${i}`));
    ...
    const render = () => {
        ...
        for (const spell of filtered.slice(0, 80)) {
            ...
            const details = el(document, "details", "");
            details.append(el(document, "summary", "", `${spell.name ?? spell.id} · ${Number(spell["level"]) === 0 ? "Cantrip" : `Level ${String(spell["level"] ?? "?")}`} ${label(spell["school"])}`));
            details.append(el(document, "p", "", typeof spell["text"] === "string" ? spell["text"] : "No description available."));
            row.append(details);
            ...
            if (Number(spell["level"]) === 0 || (state.cantrips[classID] ?? []).includes(spell.id))
                choose("cantrips", "Learn cantrip", "Forget cantrip");
            else {
                if (isBook)
                    choose("spellbook", "Learn spell", "Forget spell");
            ...
            if (castable && spell["unavailable"] !== true) {
                ...
                slots.setAttribute("aria-label", `Casting slot for ${spell.name ?? spell.id}`);
                if (Number(spell["level"]) === 0)
                    slots.append(option(document, "", "No slot needed"));
                else
                    for (const resource of rows(computed["resources"]).filter(item => item["kind"] === "slot")) {
    ...
    root.append(controls, summary, results);
    render();
    return root;
}
export function foundationEditor(view, plan, species, backgrounds, update) {
    ...
    root.append(el(document, "h4", "", "Foundation"));
    const fields = el(document, "div", "dnd-sheet-form-grid");
    for (const [field, title, catalog] of [["species", "Species", species], ["background", "Background", backgrounds]]) {
        const wrapper = el(document, "label", "dnd-sheet-field", title), select = el(document, "select", "");
        select.setAttribute("aria-label", title);
        select.disabled = !editable;
        select.append(option(document, "", `Choose ${title.toLowerCase()}…`));
        ...
        if (stored && !known)
            select.append(option(document, stored, `${stored} (saved)`));
    ...
    if (lineages.length || state.lineage) {
        const wrapper = el(document, "label", "dnd-sheet-field", "Lineage"), select = el(document, "select", "");
        select.setAttribute("aria-label", "Lineage");
        select.disabled = !editable;
        select.append(option(document, "", "Choose lineage…"));
        ...
        if (state.lineage && !lineages.some(item => item["id"] === state.lineage))
            select.append(option(document, state.lineage, `${state.lineage} (saved)`));
    ...
    const manualLabel = el(document, "label", "dnd-sheet-field", "Rolled / manual scores"), manual = el(document, "input", "");
    ...
    for (const ability of abilities) {
        const wrapper = el(document, "label", "dnd-sheet-field", `Base ${ability}`), input = el(document, "input", "");
        input.type = "number";
