...
export function builderProgress(document, guidance, navigate, open, toggle) {
    const rail = el(document, "details", "dse-build-rail");
    rail.setAttribute("aria-label", "Build progress");
    rail.open = open;
    ...
    progress.setAttribute("aria-label", "Build completion");
    summary.append(progress, el(document, "strong", "", remaining ? `${remaining} ${remaining === 1 ? "choice" : "choices"} remaining` : "Build choices complete"));
    rail.append(summary);
    const titles = { foundation: "Foundation", progression: "Class progression", spells: "Spell choices" };
...
export function builderTabs(document, classes, active, navigate) {
    const tabs = [{ classId: "character", name: "Character" }, ...classes], nav = el(document, "div", "codex-tab-strip dnd-builder-tabs");
...
export function builderSummary(document, guidance) {
...
export function extraFeats(view, catalog, change, add) {
    ...
    section.append(el(document, "h4", "", "Extra feats & rewards"));
    for (const feat of state.extraFeats) {
        ...
        note.maxLength = 500;
        note.setAttribute("aria-label", `${name} source note`);
        ...
        row.append(note);
        if (editable)
            row.append(button(document, "Remove", () => change(draft => { draft.extraFeats = draft.extraFeats.filter(value => value["id"] !== feat["id"]); }), false, `Remove extra feat ${name}`));
        section.append(row);
    }
    if (!state.extraFeats.length)
        section.append(el(document, "p", "dse-empty", "No extra feats or rewards."));
    if (editable)
        section.append(button(document, "Add extra feat or reward", add));
    return section;
}
export function extraFeatForm(document, catalog, submit, dirty) {
    const form = el(document, "form", "dnd-extra-feat-form");
    const selected = el(document, "select", "");
    selected.setAttribute("aria-label", "Extra feat");
    selected.append(new Option("Custom reward…", ""));
    ...
    name.placeholder = "Custom reward name";
    name.setAttribute("aria-label", "Custom reward name");
    name.maxLength = 200;
    name.required = true;
    note.placeholder = "Source note";
    note.setAttribute("aria-label", "Source note");
    note.maxLength = 500;
    selected.addEventListener("change", () => { name.disabled = selected.value !== ""; name.required = selected.value === ""; dirty(); });
    form.addEventListener("input", dirty);
    const apply = button(document, "Add reward", () => { });
    apply.type = "submit";
