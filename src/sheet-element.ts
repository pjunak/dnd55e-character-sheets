...
import { abilities, abilityModifier, cloneSheet, createId, signed, skills, type Ability, type InventoryItem, type ResourceItem, type SheetState, type SpellItem } from "./sheet-state.js";
import type { SheetSnapshot } from "./sheet-repository.js";

export const sheetElementTag = "dnd-character-sheet";
type Tab = "sheet" | "combat" | "spells" | "inventory" | "builder" | "notes" | "tools";
const tabs: readonly (readonly [Tab, string])[] = [["sheet", "Sheet"], ["combat", "Combat"], ["spells", "Spells"], ["inventory", "Inventory"], ["builder", "Builder"], ["notes", "Notes"], ["tools", "Tools"]];

export function defineSheetElement(): void {
  if (customElements.get(sheetElementTag) !== undefined) return;
  class CharacterSheetElement extends HTMLElement {
    ...
    set codexContribution(value: ContributionContext) {
      ...
      if (this.isConnected) {
        if (previousKey !== value.host.key) void this.#connect();
        else this.#render();
      }
    }

    connectedCallback(): void { this.classList.add("addon-dnd-sheets"); void this.#connect(); }
    ...
    async #connect(): Promise<void> {
      ...
      this.#busy = true;
      this.#render();
      try { this.#snapshot = await runtime.repository.load(contribution.host.key); }
      catch (error) { this.#fail(error, "Could not load this character sheet."); }
    ...
    #render(): void {
      ...
      if (this.#message.length > 0) root.append(messageBlock(document, this.#message, this.#messageKind));
      ...
      switch (this.#tab) {
        ...
        case "inventory": this.#renderInventory(panel, snapshot.state); break;
    ...
    #renderSheet(panel: HTMLElement, state: SheetState): void {
      ...
      const scoreGrid = document.createElement("div"); scoreGrid.className = "dnd-sheet-abilities";
      for (const ability of abilities) {
        const tile = document.createElement("label");
        const name = document.createElement("span"); name.textContent = ability;
        ...
        const modifier = document.createElement("strong"); modifier.textContent = signed(abilityModifier(state.abilities[ability]));
        tile.append(name, input, modifier); scoreGrid.append(tile);
      }
      scores.append(scoreGrid);
      const proficiencies = card(document, "Proficiencies");
      const columns = document.createElement("div"); columns.className = "dnd-sheet-proficiencies";
      const saves = document.createElement("div");
      const saveTitle = document.createElement("h4"); saveTitle.textContent = "Saving throws"; saves.append(saveTitle);
      for (const ability of abilities) saves.append(this.#proficiencyToggle(ability, state.saveProf[ability] === true, (draft, checked) => { draft.saveProf[ability] = checked; draft.manualSaveProf[ability] = checked; }, signed(abilityModifier(state.abilities[ability]) + (state.saveProf[ability] ? state.profBonus : 0))));
      const skillList = document.createElement("div");
      const skillTitle = document.createElement("h4"); skillTitle.textContent = "Skills"; skillList.append(skillTitle);
      for (const [skill, ability] of skills) {
        ...
        skillList.append(this.#proficiencyToggle(titleCase(skill), state.skillProf[skill] === true, (draft, checked) => { draft.skillProf[skill] = checked; if (!checked) draft.skillExpertise[skill] = false; }, `${signed(bonus)}${expertise ? " expertise" : ""}`, (draft) => { draft.skillExpertise[skill] = !draft.skillExpertise[skill]; draft.skillProf[skill] = true; }));
      }
      columns.append(saves, skillList); proficiencies.append(columns);
    ...
    #renderCombat(panel: HTMLElement, state: SheetState): void {
      ...
      const grid = document.createElement("div"); grid.className = "dnd-sheet-vitals";
      grid.append(
        this.#numberField("Current HP", state.hp, (draft, value) => { draft.hp = Math.max(0, Math.min(value, Math.max(draft.maxHp, value))); }, 0),
        this.#numberField("Maximum HP", state.maxHp, (draft, value) => { draft.maxHp = Math.max(0, value); draft.hp = Math.min(draft.hp, draft.maxHp); }, 0),
        this.#numberField("Temporary HP", state.tempHp, (draft, value) => { draft.tempHp = Math.max(0, value); }, 0),
        this.#numberField("Armor class", state.ac, (draft, value) => { draft.ac = value; }),
        this.#numberField("Initiative", state.initiative, (draft, value) => { draft.initiative = value; }),
        this.#numberField("Speed", state.speed, (draft, value) => { draft.speed = Math.max(0, value); }, 0),
        this.#numberField("Proficiency", state.profBonus, (draft, value) => { draft.profBonus = value; }),
      );
      vitals.append(grid);
      const resources = card(document, "Resources");
      resources.append(this.#resourceTable(state.resources));
      if (this.#canEdit()) resources.append(actionButton(document, "Add resource", () => void this.#save((draft) => { draft.resources.push({ id: createId("resource"), name: "New resource", current: 0, max: 1 }); })));
    ...
    #renderInventory(panel: HTMLElement, state: SheetState): void {
      const document = panel.ownerDocument;
      const currency = card(document, "Currency");
      const coins = document.createElement("div"); coins.className = "dnd-sheet-currency";
      for (const coin of ["pp", "gp", "ep", "sp", "cp"] as const) coins.append(this.#numberField(coin.toUpperCase(), state.currency[coin] ?? 0, (draft, value) => { draft.currency[coin] = Math.max(0, value); }, 0));
      currency.append(coins);
      const inventory = card(document, "Inventory");
      inventory.append(this.#inventoryTable(state.inventory));
      if (this.#canEdit()) inventory.append(actionButton(document, "Add item", () => void this.#save((draft) => { draft.inventory.push({ id: createId("item"), name: "New item", qty: 1, location: "pack", notes: "" }); })));
    ...
    #renderTools(panel: HTMLElement, state: SheetState): void {
      ...
      const explanation = document.createElement("p"); explanation.textContent = "Export or import only this character's D&D sheet data. Campaign conversion is handled separately during the rewrite cutover.";
    ...
    #inventoryTable(items: readonly InventoryItem[]): HTMLElement { return this.#editableTable(items, ["name", "qty", "location", "notes"], (draft) => draft.inventory); }
    ...
    #proficiencyToggle(label: string, checked: boolean, update: (draft: SheetState, checked: boolean) => void, value: string, expertise?: (draft: SheetState) => void): HTMLElement {
      const wrapper = this.ownerDocument.createElement("div"); wrapper.className = "dnd-sheet-prof-row";
      const input = this.ownerDocument.createElement("input"); input.type = "checkbox"; input.checked = checked; input.disabled = !this.#canEdit(); input.addEventListener("change", () => void this.#save((draft) => update(draft, input.checked)));
      const text = this.ownerDocument.createElement("span"); text.textContent = label;
      const total = this.ownerDocument.createElement("strong"); total.textContent = value;
      wrapper.append(input, text, total);
      if (expertise !== undefined && this.#canEdit()) wrapper.append(actionButton(this.ownerDocument, "Expertise", () => void this.#save(expertise), "small"));
      return wrapper;
    }

    async #save(mutate: (draft: SheetState) => void): Promise<void> {
      const runtime = this.#runtime; const snapshot = this.#snapshot;
      if (runtime === undefined || snapshot === undefined || !this.#canEdit() || this.#busy) return;
      this.#busy = true; this.#message = "Saving…"; this.#messageKind = "status"; this.#render();
      try { this.#snapshot = await runtime.repository.save(snapshot, mutate); this.#message = "Saved."; }
      catch (error) {
        if (errorStatus(error) === 409) { try { this.#snapshot = await runtime.repository.load(snapshot.key); } catch { /* original conflict remains most useful */ } this.#message = "The sheet changed elsewhere. The newest version was reloaded; please make your edit again."; }
        else this.#message = errorMessage(error, "Could not save the sheet.");
        this.#messageKind = "alert";
      } finally { this.#busy = false; this.#render(); }
    }

    async #loadBuilder(): Promise<void> {
      const runtime = this.#runtime; const snapshot = this.#snapshot; if (runtime === undefined || snapshot === undefined || this.#busy) return;
      this.#busy = true; this.#message = "Loading builder…"; this.#render();
      try {
      ...
      } catch (error) { this.#fail(error, "Could not load the builder."); }
      finally { this.#busy = false; this.#render(); }
    }

    async #changeClasses(classes: readonly Record<string, unknown>[]): Promise<void> {
      const runtime = this.#runtime; const snapshot = this.#snapshot; if (runtime === undefined || snapshot === undefined || !this.#canEdit()) return;
      ...
      try {
        ...
        const plan = await runtime.engine.builderPlan(this.#snapshot.state); this.#builderPlan = plan.plan;
        this.#hydration = hydration; this.#message = "Build updated and fallback values saved."; this.#messageKind = "status";
      } catch (error) { this.#fail(error, "Could not update the classes."); }
      finally { this.#busy = false; this.#render(); }
    }

    async #applyBuilderChoice(choiceId: string, value: unknown, slot?: number): Promise<void> {
      const runtime = this.#runtime; const snapshot = this.#snapshot; if (runtime === undefined || snapshot === undefined || !this.#canEdit() || this.#busy) return;
      this.#busy = true; this.#render();
      try {
      ...
      } catch (error) { this.#fail(error, "Could not apply this builder choice."); }
      finally { this.#busy = false; this.#render(); }
    }
    ...
    async #previewHydration(): Promise<void> {
      const runtime = this.#runtime; const snapshot = this.#snapshot; if (runtime === undefined || snapshot === undefined || this.#busy) return;
      this.#busy = true; this.#render();
      try { this.#hydration = await runtime.engine.hydrate(snapshot.state); this.#message = this.#hydration.warnings.length > 0 ? this.#hydration.warnings.join(" ") : "Computed preview refreshed."; this.#messageKind = "status"; }
      catch (error) { this.#fail(error, "Could not compute this sheet."); }
      finally { this.#busy = false; this.#render(); }
    }
    ...
    async #materialize(): Promise<void> {
      ...
      try {
        ...
        this.#snapshot = await runtime.repository.save(snapshot, (draft) => replaceState(draft, materialized)); this.#hydration = hydration; this.#message = "Computed fallback values saved."; this.#messageKind = "status";
      } catch (error) { this.#fail(error, "Could not refresh computed values."); }
      finally { this.#busy = false; this.#render(); }
    }

    async #importFile(file: File): Promise<void> {
      if (!this.#canEdit() || this.#snapshot === undefined) return;
      try { const imported = parseSheet(await file.text()); await this.#save((draft) => replaceState(draft, imported)); this.#message = "Imported sheet saved."; }
      catch (error) { this.#fail(error, "Could not import this sheet."); this.#render(); }
    }

    #canEdit(): boolean { return this.#contribution?.host.canEdit === true; }
