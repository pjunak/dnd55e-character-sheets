...
export function abilityRail(view: PlayView): HTMLElement {
  ...
  const abilityNames: Record<Ability, string> = { STR: t("ability.STR"), DEX: t("ability.DEX"), CON: t("ability.CON"), INT: t("ability.INT"), WIS: t("ability.WIS"), CHA: t("ability.CHA") };
  ...
  for (const ability of abilities) {
    ...
    if (layout === "compact" && ability === "DEX") dock.append(el(document, "span", "dse-dock", t("play.initDock", { value: signed(state.initiative) })));
    if (layout === "compact" && ability === "WIS") dock.append(el(document, "span", "dse-dock", t("play.passiveDock", { value: passive(state) })));
    title.append(dock);
    ...
    for (const [name] of governed) {
      ...
      dot.className = "dse-dot"; dot.setAttribute("aria-label", t("play.skillProficiency", { name: sheetLabel(view.locale, name), rank: t(expert ? "play.expertise" : prof ? "play.proficient" : "play.untrained") }));
    ...
    if (governed.length === 0) details.append(el(document, "span", "dse-empty", t("play.noSkills")));
...
export function vitals(view: PlayView): HTMLElement {
  ...
  counter.append(button(document, "−", () => save(draft => { draft.hp = Math.max(0, draft.hp - 1); }), !editable, t("play.hpDown")),
    number(view, t("play.hpCurrent"), state.hp, (draft, value) => { draft.hp = Math.max(0, value); }, 0),
    el(document, "span", "", "/"), number(view, t("play.hpMax"), state.maxHp, (draft, value) => { draft.maxHp = Math.max(0, value); }, 0),
    button(document, "+", () => save(draft => { draft.hp += 1; }), !editable, t("play.hpUp")));
  const temp = el(document, "label", "dse-temp"); temp.append(el(document, "span", "", t("play.temp")), number(view, t("play.hpTemp"), state.tempHp, (draft, value) => { draft.tempHp = Math.max(0, value); }, 0));
  hp.append(counter, temp); band.append(hp);
  const ac = el(document, "label", "codex-tile dse-ac"); ac.append(el(document, "span", "dse-stat-label", t("play.ac")), number(view, t("play.ac"), state.ac, (draft, value) => { draft.ac = value; }, 0)); band.append(ac);
  const stats = el(document, "div", "dse-vitals-grid");
  const speed = el(document, "label", "codex-tile"); speed.append(el(document, "span", "dse-stat-label", t("play.speed")), number(view, t("play.speed"), state.speed, (draft, value) => { draft.speed = Math.max(0, value); }, 0)); stats.append(speed);
  const proficiencyTile = el(document, "label", "codex-tile"); proficiencyTile.append(el(document, "span", "dse-stat-label", t("play.proficiency")), number(view, t("play.proficiency"), state.profBonus, (draft, value) => { draft.profBonus = value; })); stats.append(proficiencyTile);
  if (view.layout === "classic") {
    const initiative = el(document, "label", "codex-tile"); initiative.append(el(document, "span", "dse-stat-label", t("field.initiative")), number(view, t("field.initiative"), state.initiative, (draft, value) => { draft.initiative = value; })); stats.append(initiative);
    const perception = el(document, "div", "codex-tile"); perception.append(el(document, "span", "dse-stat-label", t("play.passive")), el(document, "strong", "", String(passive(state)))); stats.append(perception);
    for (const caster of rows(record(savedComputed(state)["spellcasting"])["perClass"])) {
      for (const [name, value, attack] of [[t("play.saveDC"), caster["saveDC"], false], [t("play.spellAttack"), caster["spellAttack"], true]] as const) {
        const stat = el(document, "div", "codex-tile"); stat.title = String(caster["classId"] ?? ""); stat.append(el(document, "span", "dse-stat-label", name), el(document, "strong", "", attack ? signed(numeric(value, 0)) : String(numeric(value, 0)))); stats.append(stat);
      }
    }
  }
  band.append(stats);
  ...
  if (worn.length > 0 || editable) {
    const slots = el(document, "div", "dse-worn"); slots.append(el(document, "span", "dse-stat-label", t("play.wornHeading")));
    for (const [slot, title] of [["armor", t("play.armor")], ["shield", t("play.shield")], ["worn", t("play.worn")], ["attuned", t("play.attuned")]] as const) {
      ...
      for (const item of items) {
        ...
        if (editable && view.clearSlot) token.append(button(document, "×", () => view.clearSlot!(item.id), false, t(slot === "attuned" ? "play.endAttunement" : "play.unequip", { name: item.name }))); group.append(token);
      }
      if (editable && view.fillSlot) group.append(button(document, `＋ ${title}`, () => view.fillSlot!(slot), false, t("play.fillSlot", { name: title })));
...
export function backpack(view: PlayView): HTMLElement {
  ...
  const title = el(document, "h3", "dse-bp-title"); title.append(icon(document, "M8 6V4a4 4 0 0 1 8 0v2M5 6h14v15H5ZM5 10h14M9 10v3h6v-3"), document.createTextNode(t("play.backpack"))); heading.append(title);
  if (editable) heading.append(button(document, t("play.addItem"), view.addItem ?? (() => save(draft => { draft.inventory.push({ id: createId("item"), name: t("play.newItem"), qty: 1, location: "pack", notes: "" }); }))));
  pack.append(heading);
  ...
  for (const location of locations) {
    ...
    const group = el(document, "div", "dse-bp-group"); group.append(el(document, "h4", "dse-bp-label", `${sheetLabel(view.locale, location)} · ${items.length}`));
    for (const item of items) {
      ...
      if (!editable) row.append(el(document, "span", "dse-item-name", item.name), el(document, "span", "", `× ${item.qty}${item["attuned"] ? " ★" : ""}`));
        ...
        const name = el(document, "input", "dse-item-name"); name.value = item.name; name.setAttribute("aria-label", t("play.itemName"));
        ...
        const qty = number(view, t("play.quantity", { name: item.name }), item.qty, (draft, value) => { const target = draft.inventory.find(value => value.id === item.id); if (target) target.qty = Math.max(0, value); }, 0);
        const attune = button(document, item["attuned"] ? "★" : "☆", () => save(draft => { const target = draft.inventory.find(value => value.id === item.id); if (target) target["attuned"] = !target["attuned"]; }), false, t("play.attune", { name: item.name })); attune.setAttribute("aria-pressed", String(item["attuned"] === true));
        const move = el(document, "select", "dse-item-location"); move.setAttribute("aria-label", t("play.move", { name: item.name }));
        for (const loc of locations) { const option = el(document, "option", "", sheetLabel(view.locale, loc)); option.value = loc; move.append(option); } move.value = location;
        ...
        notes.value = item.notes; notes.rows = 2; notes.setAttribute("aria-label", t("play.notes", { name: item.name }));
    ...
    if (items.length === 0) group.append(el(document, "p", "dse-empty", t("common.empty")));
...
export function combatDetails(view: PlayView): HTMLElement {
  ...
  for (const weapon of rows(computed["weapons"])) {
    const row = el(document, "div", "dse-attack"); row.append(el(document, "strong", "", String(weapon["name"] ?? weapon["ref"] ?? t("play.weapon"))), el(document, "span", "", signed(numeric(weapon["attackBonus"], 0))), el(document, "span", "", String(weapon["damage"] ?? ""))); attacks.append(row);
  }
  if (rows(computed["weapons"]).length === 0) attacks.append(el(document, "p", "dse-empty", t("play.noAttacks")));
  body.append(attacks);
  ...
  for (const resource of state.resources) {
    ...
    row.append(button(document, "−", () => save(draft => update(draft, -1)), !editable, t("play.use", { name: resource.name })), number(view, resource.name, resource.current, (draft, value) => { const target = draft.resources.find(item => item.id === resource.id); if (target) target.current = value; }, 0, resource.max), el(document, "span", "", `/ ${resource.max}`), button(document, "+", () => save(draft => update(draft, 1)), !editable, t("play.recover", { name: resource.name }))); resources.append(row);
  }
  if (resources.children.length === 1) resources.append(el(document, "p", "dse-empty", t("play.noResources")));
  ...
  for (const key of ["languages", "resistances", "damageImmunities", "conditionImmunities"]) {
    const entries = values[key]; if (Array.isArray(entries) && entries.length > 0) traits.append(el(document, "p", "", `${sheetLabel(view.locale, key)}: ${entries.filter(entry => typeof entry === "string").join(", ")}`));
  }
  const profs = record(computed["proficiencies"]);
  for (const key of ["armor", "weapons", "tools"]) {
    const entries = profs[key] ?? state.traitSnapshot[key]; if (Array.isArray(entries) && entries.length > 0) traits.append(el(document, "p", "", `${sheetLabel(view.locale, key)}: ${entries.join(", ")}`));
  }
