...
export function castingSlots(document: Document, name: string, keys: readonly string[], options: Record<string, unknown>): HTMLSelectElement {
  const select = el(document, "select", ""); select.setAttribute("aria-label", `Casting slot for ${name}`);
...
export function grantedSpells(view: PlayView, catalog: readonly RuleRecord[], options: Record<string, unknown>, change: (value: PlayChange) => void): HTMLElement {
  ...
  const root = el(document, "section", "dnd-granted-spells dse-section"); root.append(el(document, "h3", "", "Species & feat spells"));
  for (const choice of rows(options["castingAbilityChoices"])) {
    ...
    select.setAttribute("aria-label", `${name} casting ability`); select.disabled = !editable; select.append(option(document, "", "Choose ability…"));
  ...
  for (const choice of rows(options["pendingChoices"])) {
    ...
    const group = el(document, "fieldset", "dnd-spell-grant-choice"); group.dataset["grant"] = key; group.append(el(document, "legend", "", `${name} · ${picked.length}/${String(choice["choose"])} spells`));
    for (const ref of picked) group.append(button(document, `Remove ${catalog.find(item => item.id === ref)?.name ?? ref}`, () => change({ operation: "select-grant-spell", key, ref, selected: false }), !editable));
    const controls = el(document, "div", "dnd-workflow-controls"), select = el(document, "select", ""); select.setAttribute("aria-label", `Spell choice ${key}`); select.disabled = !editable;
    ...
    controls.append(select, button(document, "Choose spell", () => change({ operation: "select-grant-spell", key, ref: select.value, selected: true }), !editable || !select.options.length || picked.length >= Number(choice["choose"]))); group.append(controls); root.append(group);
  }
  for (const grant of rows(options["granted"])) {
    ...
    controls.append(select, button(document, "Cast granted spell", () => change({ operation: "cast-granted-spell", key, slot: select.value }), !editable || select.selectedIndex < 0)); row.append(controls); root.append(row);
  }
  if (root.children.length === 1) root.append(el(document, "p", "dse-empty", "No spell grants in this build."));
  return root;
}

export function copySpellForm(document: Document, state: SheetState, spell: RuleRecord, classId: string, cost: number, submit: (change: PlayChange) => void): HTMLElement {
  ...
  const scroll = el(document, "select", ""); scroll.setAttribute("aria-label", "Scroll to consume"); scroll.append(option(document, "", "Copy from another spellbook (no scroll)"));
  const foldedName = (spell.name ?? spell.id).toLocaleLowerCase();
  for (const item of state.inventory) if (item.qty > 0 && (item["spellRef"] === spell.id || item.name.toLocaleLowerCase().includes("scroll") && item.name.toLocaleLowerCase().includes(foldedName))) scroll.append(option(document, item.id, `${item.name} · ${item.qty} remaining`));
  const apply = button(document, "Review copying", () => {}, Number(state.currency["gp"] ?? 0) < cost); apply.type = "submit";
...
export function swapSpellForm(document: Document, state: SheetState, classId: string, eligible: readonly string[], catalog: readonly RuleRecord[], submit: (change: PlayChange) => void): HTMLElement {
  const form = el(document, "form", "dnd-spell-swap dnd-play-review"), selected = state.preparedSpells[classId] ?? [];
  const from = el(document, "select", ""), to = el(document, "select", ""); from.setAttribute("aria-label", "Replace spell"); to.setAttribute("aria-label", "Replacement spell");
  ...
  const apply = button(document, "Review spell swap", () => {}, !from.options.length || !to.options.length); apply.type = "submit";
  form.addEventListener("submit", event => { event.preventDefault(); submit({ operation: "swap-spell", classId, out: from.value, ref: to.value }); });
  form.append(el(document, "p", "", "Replace a known spell and record the change at the current class level."), from, to, apply); return form;
}

export function spellSwapHistory(view: PlayView, catalog: readonly RuleRecord[]): HTMLElement {
  const { document, state, editable, save } = view, section = el(document, "details", "dnd-spell-history"); section.append(el(document, "summary", "", `Level-up spell changes · ${state.spellSwaps.length}`));
  state.spellSwaps.forEach((swap, index) => {
    ...
    if (editable) row.append(button(document, "Remove history entry", () => save(draft => { draft.spellSwaps.splice(index, 1); }))); section.append(row);
  }); return section;
}
