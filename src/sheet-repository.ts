import type { AddonDataHandle } from "./sdk.js";
import { blankSheet, cloneSheet, normalizeSheet, type SheetState } from "./sheet-state.js";
...
export class SheetRepository {
  readonly #handle: AddonDataHandle<SheetState>;
  readonly #signal: AbortSignal;
  #writeTail: Promise<void> = Promise.resolve();

  constructor(handle: AddonDataHandle<SheetState>, signal: AbortSignal) {
    this.#handle = handle;
    this.#signal = signal;
  }

  async load(key: string): Promise<SheetSnapshot> {
    ...
    try {
      ...
      if (httpStatus(error) === 404) return { key, revision: 0, state: blankSheet() };
      throw error;
    }
  }

  save(snapshot: SheetSnapshot, mutate: (draft: SheetState) => void): Promise<SheetSnapshot> {
    const operation = this.#writeTail.then(async () => {
      ...
      if (result === undefined) throw new Error("The host did not acknowledge the sheet write.");
