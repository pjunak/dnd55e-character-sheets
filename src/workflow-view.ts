...
export function equipmentPicker(document: Document, catalog: readonly RuleRecord[], unavailable: boolean, dirty: (value: boolean) => void, commit: (items: SheetState["inventory"]) => void, t: SheetTranslator = sheetTranslator()): HTMLElement {
  const categoryNames: Record<string, string> = { weapon: t("equipment.weapons"), armor: t("play.armor"), gear: t("equipment.gear"), tool: t("equipment.tools"), "magic-item": t("equipment.magic"), pack: t("equipment.packs") };
  const root = el(document, "div", "dnd-equipment-picker");
  const controls = el(document, "div", "dnd-workflow-controls");
  let path: string[] = [];
  const breadcrumbs = el(document, "nav", "dnd-equipment-path"); breadcrumbs.setAttribute("aria-label", t("equipment.folders"));
  const search = el(document, "input", ""); search.type = "search"; search.placeholder = t("equipment.searchPlaceholder"); search.setAttribute("aria-label", t("equipment.search"));
  ...
  const renderTray = (): void => {
    tray.replaceChildren(el(document, "h4", "", t("equipment.selected", { count: cart.length }))); add.disabled = cart.length === 0;
    dirty(cart.length > 0);
    for (const item of cart) {
      const row = el(document, "div", "dnd-picker-row"); row.append(el(document, "span", "", item.name));
      const qty = el(document, "input", ""); qty.type = "number"; qty.min = "1"; qty.max = "5000"; qty.value = String(item.qty); qty.setAttribute("aria-label", t("equipment.quantity", { name: item.name }));
      qty.addEventListener("change", () => { item.qty = Math.min(5000, Math.max(1, Math.trunc(qty.valueAsNumber || 1))); qty.value = String(item.qty); });
      row.append(qty, button(document, "×", () => { cart.splice(cart.indexOf(item), 1); renderTray(); }, false, t("equipment.removeSelected", { name: item.name }))); tray.append(row);
    }
    if (!cart.length) tray.append(el(document, "p", "dse-empty", t("equipment.trayHelp")));
  };
  ...
  const renderResults = (): void => {
    ...
    if (!query && (!kind || path.length <= facets.length)) {
      ...
      for (const value of groups) {
        ...
        const folder = button(document, `▸ ${title} (${count})`, () => { path = [...path, value]; renderResults(); }, false, t("equipment.openFolder", { name: title })); folder.className = "dnd-equipment-folder"; results.append(folder);
      }
      return;
    }
    for (const item of filtered.slice(0, 80)) {
      ...
      row.append(details, button(document, "＋", () => stage(item, item.name ?? item.id), false, t("equipment.select", { name: item.name ?? item.id }))); results.append(row);
    }
  };
  search.addEventListener("input", renderResults);
  const custom = el(document, "form", "dnd-workflow-controls"); const name = el(document, "input", ""); name.required = true; name.maxLength = 200; name.placeholder = t("equipment.customName"); name.setAttribute("aria-label", t("equipment.customName"));
  const customAdd = button(document, t("equipment.selectCustom"), () => {}); customAdd.type = "submit";
  ...
  if (unavailable) root.append(el(document, "p", "dse-empty", t("equipment.unavailable")));
...
export function restControls(view: PlayView, available: boolean, change: (value: PlayChange, review?: boolean) => void): HTMLElement {
  ...
  for (const rest of ["short", "long"] as const) buttons.append(button(document, rest === "short" ? t("rest.short") : t("rest.long"), () => change({ operation: "rest", rest }, true), !editable || !available));
  ...
  if (!available) controls.append(el(document, "p", "dse-empty", t("rest.unavailable")));
  return controls;
}

export function playReview(document: Document, before: SheetState, after: SheetState, t: SheetTranslator = sheetTranslator()): HTMLElement {
  ...
  if (ended.length) review.append(el(document, "p", "", t("rest.ended", { count: ended.length })));
  return review;
}
...
export function spellBrowser(view: PlayView, catalog: readonly RuleRecord[], computed: Record<string, unknown>, browser: SpellBrowserState, change: (value: PlayChange) => void, tools: SpellActions): HTMLElement {
  ...
  const root = el(document, "section", "dnd-spell-browser dse-section"); root.append(el(document, "h3", "", t("spells.manage")));
  const casters = rows(record(computed["spellcasting"])["perClass"]);
  if (!casters.length) { root.append(el(document, "p", "dse-empty", t("spells.chooseClass"))); return root; }
  if (!casters.some(caster => caster["classId"] === browser.classId)) browser.classId = String(casters[0]!["classId"]);
  const classSelect = el(document, "select", ""); classSelect.setAttribute("aria-label", t("spells.class"));
  ...
  const search = el(document, "input", ""); search.type = "search"; search.placeholder = t("spells.searchPlaceholder"); search.setAttribute("aria-label", t("spells.search")); search.value = browser.query;
  const level = el(document, "select", ""); level.setAttribute("aria-label", t("spells.level")); level.append(option(document, "", t("spells.allLevels"))); for (let i = 0; i <= 9; i++) level.append(option(document, String(i), i === 0 ? t("spells.cantrips") : t("spells.levelNumber", { level: i }))); level.value = browser.level;
  ...
  const render = (): void => {
    ...
    if (options["canSwap"]) classActions.append(button(document, t("spells.swapLevel"), () => tools.swap(classID, eligible), !editable));
    ...
    for (const spell of filtered.slice(0, 80)) {
      ...
      const details = el(document, "details", ""); details.append(el(document, "summary", "", `${spell.name ?? spell.id} · ${Number(spell["level"]) === 0 ? t("spells.cantrip") : t("spells.levelNumber", { level: String(spell["level"] ?? "?") })} ${label(spell["school"])}`));
      details.append(el(document, "p", "", typeof spell["text"] === "string" ? spell["text"] : t("common.noDescription"))); row.append(details);
      ...
      if (Number(spell["level"]) === 0 || (state.cantrips[classID] ?? []).includes(spell.id)) choose("cantrips", t("spells.learnCantrip"), t("spells.forgetCantrip"));
      else {
        if (isBook) choose("spellbook", t("spells.learn"), t("spells.forget"));
      ...
      if (castable && spell["unavailable"] !== true) {
        const slots = el(document, "select", ""); slots.setAttribute("aria-label", t("spells.castingSlot", { name: spell.name ?? spell.id }));
        if (Number(spell["level"]) === 0) slots.append(option(document, "", t("spells.noSlot")));
      ...
      if ((options["ritualIds"] as string[] | undefined)?.includes(spell.id)) actions.append(button(document, t("spells.ritual"), () => change({ operation: "cast-ritual", classId: classID, ref: spell.id }), !editable));
      const cost = record(options["copyCosts"])[spell.id];
      if (typeof cost === "number" && !(state.spellbook[classID] ?? []).includes(spell.id)) actions.append(button(document, t("spells.copyPrice", { cost }), () => tools.copy(spell, classID, cost), !editable));
...
export function foundationEditor(view: PlayView, plan: BuilderPlan, species: readonly RuleRecord[], backgrounds: readonly RuleRecord[], update: (change: (draft: SheetState) => void) => void): HTMLElement {
  const t = sheetTranslator(view.locale);
  const { document, state, editable } = view;
  const root = el(document, "section", "dnd-builder-foundation"); root.append(el(document, "h4", "", t("builder.foundation")));
  const fields = el(document, "div", "dnd-sheet-form-grid");
  for (const [field, title, catalog] of [["species", t("field.species"), species], ["background", t("field.background"), backgrounds]] as const) {
    const wrapper = el(document, "label", "dnd-sheet-field", title), select = el(document, "select", ""); select.setAttribute("aria-label", title); select.disabled = !editable; select.append(option(document, "", t(field === "species" ? "builder.chooseSpecies" : "builder.chooseBackground")));
    ...
    if (stored && !known) select.append(option(document, stored, t("common.savedValue", { value: stored }))); select.value = known?.name ?? stored;
  ...
  if (lineages.length || state.lineage) {
    const wrapper = el(document, "label", "dnd-sheet-field", t("builder.lineage")), select = el(document, "select", ""); select.setAttribute("aria-label", t("builder.lineage")); select.disabled = !editable;
    select.append(option(document, "", t("builder.chooseLineage"))); for (const item of lineages) select.append(option(document, String(item["id"]), String(item["name"] ?? item["id"])));
    if (state.lineage && !lineages.some(item => item["id"] === state.lineage)) select.append(option(document, state.lineage, t("common.savedValue", { value: state.lineage }))); select.value = state.lineage;
  ...
  const manualLabel = el(document, "label", "dnd-sheet-field", t("builder.manualScores")), manual = el(document, "input", ""); manual.type = "checkbox"; manual.checked = state.manualScores; manual.disabled = !editable;
  ...
  for (const ability of abilities) {
    const wrapper = el(document, "label", "dnd-sheet-field", t("builder.baseAbility", { ability })), input = el(document, "input", ""); input.type = "number"; input.value = String(scores[ability]); input.min = String(range["min"]); input.max = String(range["max"]); input.disabled = !editable;
