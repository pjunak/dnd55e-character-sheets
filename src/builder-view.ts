import { sheetTranslator, remainingChoices, type SheetTranslator } from "./sheet-catalogs.js";
...
export function builderProgress(document: Document, guidance: Record<string, unknown>, navigate: (target: BuilderTarget) => void, open: boolean, toggle: (open: boolean) => void, t: SheetTranslator = sheetTranslator()): HTMLElement {
  const rail = el(document, "details", "dse-build-rail"); rail.setAttribute("aria-label", t("builder.progress")); rail.open = open;
  ...
  const progress = el(document, "progress", "dse-build-meter"); progress.max = total || 1; progress.value = complete; progress.setAttribute("aria-label", t("builder.completion"));
  summary.append(progress, el(document, "strong", "", remaining ? remainingChoices(t, remaining) : t("builder.complete"))); rail.append(summary);
  const titles: Record<string, string> = { foundation: t("builder.foundation"), progression: t("builder.progression"), spells: t("builder.spellChoices") };
...
export function builderTabs(document: Document, classes: readonly Record<string, unknown>[], active: string, navigate: (target: BuilderTarget) => void, t: SheetTranslator = sheetTranslator()): HTMLElement {
  const tabs = [{ classId: "character", name: t("builder.character") }, ...classes], nav = el(document, "div", "codex-tab-strip dnd-builder-tabs"); nav.setAttribute("role", "tablist"); nav.setAttribute("aria-label", t("builder.sections"));
...
export function builderSummary(document: Document, guidance: Record<string, unknown>, t: SheetTranslator = sheetTranslator()): HTMLElement {
...
export function extraFeats(view: PlayView, catalog: readonly RuleRecord[], change: (mutate: (draft: SheetState) => void) => void, add: () => void): HTMLElement {
  ...
  const { document, state, editable } = view, section = el(document, "section", "dnd-extra-feats"); section.append(el(document, "h4", "", t("builder.rewards")));
  for (const feat of state.extraFeats) {
    ...
    const note = el(document, "input", ""); note.type = "text"; note.value = String(feat["sourceNote"] ?? ""); note.maxLength = 500; note.setAttribute("aria-label", t("builder.rewardSource", { name })); note.disabled = !editable;
    ...
    if (editable) row.append(button(document, t("common.remove"), () => change(draft => { draft.extraFeats = draft.extraFeats.filter(value => value["id"] !== feat["id"]); }), false, t("builder.removeReward", { name }))); section.append(row);
  }
  if (!state.extraFeats.length) section.append(el(document, "p", "dse-empty", t("builder.noRewards")));
  if (editable) section.append(button(document, t("builder.addReward"), add)); return section;
}

export function extraFeatForm(document: Document, catalog: readonly RuleRecord[], submit: (feat: Record<string, unknown>) => void, dirty: () => void, t: SheetTranslator = sheetTranslator()): HTMLElement {
  const form = el(document, "form", "dnd-extra-feat-form");
  const selected = el(document, "select", ""); selected.setAttribute("aria-label", t("builder.extraFeat")); selected.append(new Option(t("builder.customReward"), ""));
  for (const feat of catalog) selected.append(new Option(feat.name ?? feat.id, feat.id));
  const name = el(document, "input", ""), note = el(document, "input", ""); name.placeholder = t("builder.rewardName"); name.setAttribute("aria-label", t("builder.rewardName")); name.maxLength = 200; name.required = true;
  note.placeholder = t("builder.sourceNote"); note.setAttribute("aria-label", t("builder.sourceNote")); note.maxLength = 500;
  ...
  const apply = button(document, t("builder.addRewardSubmit"), () => {}); apply.type = "submit";
